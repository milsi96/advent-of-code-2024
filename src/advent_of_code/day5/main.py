...
def get_sum_invalid_updates(file_name: str) -> int:
  ...
  def _compare(rules: List[Rule], num1: int, num2: int) -> int:
    applicable_rules = list(filter(lambda rule: (num1 == rule.after and num2 == rule.before) or (num1 == rule.before and num2 == rule.after), rules))
  ...
  sorted_updates = list(map(lambda u: sorted(u, key=cmp_to_key(partial(_compare, rules))), filter(lambda u: not partial(_is_update_valid, rules)(u), updates)))
