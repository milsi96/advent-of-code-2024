...
def next_move(input: Dict[Coordinate, int], current_point: Coordinate) -> Set[Coordinate]:
  new_moves = {(current_point[X]-1, current_point[Y]), 
...
def _trigger_next_move(input: Dict[Coordinate, int], path: List[Coordinate], next_move: Coordinate) -> List[Coordinate]:
    new_path = path.copy()
    new_path.append(next_move)
    return get_trailheads(input=input, path=new_path)

def get_trailheads(input: Dict[Coordinate, int], path: List[Coordinate]) -> List[Coordinate]:
  ...
  return reduce(_reduce_list, [_trigger_next_move(input=input, path=path, next_move=move) for move in next_moves])
...
def solve_part_one(file_name: str) -> int:
  lines = process_file(file_name=file_name, process=lambda line: line.replace("\n", ""))
  ...
  result = {
    start: set(reduce(_reduce_list, [_trigger_next_move(input=input, path=[], next_move=start)]))
...
def solve_part_two(file_name: str) -> int:
  ...
  result = {
    start: reduce(_reduce_list, [_trigger_next_move(input=input, path=[], next_move=start)])
    for start in start_points
  }
