...
def get_mul_sum(file_name: str) -> int:
  return _get_multiplications(file_name=file_name, process_input=lambda lines: ''.join(lines))

def get_mul_sum_donts(file_name: str) -> int:
  def _remove_donts_substrings(line: str) -> str:
    return re.sub(r"don't\(\).*?do\(\)", "", line)
  
  return _get_multiplications(file_name=file_name, process_input=lambda lines: _remove_donts_substrings(''.join(lines).replace("\n", "")))


def _get_multiplications(file_name: str, process_input: Callable[[List[str]], str]) -> int:
