...
def _is_report_valid(report: List[int]) -> bool:
  valid_increasing = all(1 <= report[i] - report[i-1] <= 3 for i in range(1, len(report)))
  valid_decreasing = all(1 <= report[i-1] - report[i] <= 3 for i in range(1, len(report)))
  return valid_increasing or valid_decreasing

def get_safe_reports(file_name: str) -> int:
  reports: List[List[int]] = _get_input(file_name=file_name, process=_process_into_report)
  return sum([_is_report_valid(report) for report in reports])

def get_tolerated_safe_reports(file_name: str) -> int:
  reports: List[List[int]] = _get_input(file_name=file_name, process=_process_into_report)
  result = [any(_is_report_valid(report=option) for option in _get_bad_level_options(report=report)) for report in reports]
  return sum(result)
